...
<script>
	...
	function chkMileage(input, mileage) {
		...
		if (tmpMileage > mileage || tmpMileage > 10001) {
			swal("", "사용 가능한 적립금을 초과했습니다.", "error");
			input.value = "0";
			return;
		} else if (tmpMileage < 1000) {
			swal("", "1000원 이상부터 사용 가능합니다.", "error");
			input.value = "0";
			return;
		}
	...
	function useAllMileage(mileage) {
		...
		if ($("#chkAll").is(":checked") == true && mileage < 10001) {
			$("#ord_used_mileage").val(mileage);
		} else {
			swal("", "사용 가능한 적립금을 초과했습니다.", "error");
			$("#ord_used_mileage").val(10000);
			...
			swal({
				...
				$.ajax({
					...
					success : function(data) {
						...
						setTimeout('window.location.href = "main.do"', 2000);
